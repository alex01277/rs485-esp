...
//////////////////////////////////////////////////////////////////////////////////  //byte     TypeRead =1;    //1 - seqentall; 0-all zusammen
   ...
   struct  InitVal{
       ...
       byte nregs,
       byte nepr };
 InitVal   initVday,initVmonth,initVyear,initVids, initVmadr;  
 void setup() {//  p_ids=ids; p_madr=madr;
  
    eeprom_update_byte(2,pday);  //inital value
    eeprom_update_byte(4,pmonth);
    eeprom_update_byte(6,pyear);
    eeprom_update_byte(8,pmax);
  
   Serial.begin(spedss); // baud-rate at 19200
  
    if (clearRom) {
     displayEEPROM(0); Serial.println(); 
...
void loop() { //void  swing(int pdata, int ppdata, boolean b_pdata,byte nregs,byte nepr)
  ...
  swing(pday   , ppday   , b_pday  , 50 , 2 );
  if (b_pday)   ModData2[3]  = au16data[50]-cday;   // pday 
  swing(pmonth , ppmonth , b_pmonth, 52 , 4 );
  if (b_pmonth) ModData3[3]  = au16data[52]-cmonth; // pmonth
  swing(pyear  , ppyear  , b_pyear , 54 , 6 );
  if (b_pyear)   ModData4[3]  = au16data[54]-cyear;  // pyear 
  swing(pmax   , ppmax   , b_pmax  , 93 , 8 );
  if (b_pmax) {
   ModData71[3] = -au16data[93]; //pmax 
   ModData72[3] = -au16data[93]; //pmax 
   ModData73[3] = -au16data[93]; //pmax 
   ModData74[3] = -au16data[93]; }//pmax  
  swing(ids    , p_ids   , b_ids , 90 , 10);
  swing(madr   , p_madr  , b_madr, 92 , 12); 
  if (b_madr){slave.setID(madr);b_madr=0;}

   if (millis() - Previous > Interval*1000)     
    {   //reqest
      if (dbg) {Serial.println();Serial.print("get...");}
      Previous = millis();
...
InitVal  swing(int pdata, int ppdata, boolean b_pdata,byte nregs,byte nepr)
     {  ppdata   =  au16data[nregs];
        if (ppdata != pdata) {  //change  pddata acc value
           b_pdata = true;  
           pdata   = ppdata;
           eeprom_update_byte(nepr,pdata);
           if (dbg) displayEEPROM(pdata+ppdata);}  //new value
     }   
/*
    ppyear   =  au16data[54];
