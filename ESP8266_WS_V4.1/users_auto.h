...
void sensors_config() {
  ...
  sensors.add(new knob_t(0, 100, ".01", "Влажность", "г/м&sup3;"), "out_absoluteHumidity",
    [&](){ return absoluteHumidity(sensors.get("out_temperature"), sensors.get("out_humidity")); }
  );
  ...
  /* тестовый вывод */
  cron.add(cron::time_1m + cron::minute, [&](){
    #ifdef console
